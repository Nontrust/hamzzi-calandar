...
시스템은 달력에서 기념일을 일반 일정과 구분되는 방식으로 표기해야 하며, 서버에서 조회한 저장형 기념일 데이터도 동일 규칙으로 표현해야 한다. (SHALL)

#### Scenario: 동일 날짜 복수 항목 표시
- **WHEN** 같은 날짜에 일반 일정과 기념일이 함께 존재한다
- **THEN** 시스템은 기념일 전용 뱃지/점표시를 사용해 항목을 구분한다

#### Scenario: 서버 저장 기념일 렌더링
- **WHEN** 월 뷰 데이터가 서버 API에서 반환된다
- **THEN** 시스템은 저장된 기념일 항목을 로컬 계산 항목과 동일한 시각 규칙으로 렌더링한다

### Requirement: 기념일 제목 및 보조 정보 표기
시스템은 기념일 제목을 `기념일명 · D+N` 형식으로 노출하고 남은/지난 일수 정보를 함께 제공해야 하며, 기준일 원본은 서버 저장 데이터와 일치해야 한다. (SHALL)
