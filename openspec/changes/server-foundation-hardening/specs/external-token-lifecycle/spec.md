...
외부 연동 토큰(OAuth 등)의 저장, 암호화, 갱신, 폐기 수명주기를 표준화해 보안성과 안정성을 높인다.

## ADDED Requirements

### Requirement: 외부 토큰 안전 저장
시스템은 외부 연동 토큰을 평문으로 저장하지 않고 암호화된 형태로 저장해야 한다.
...
시스템은 토큰 만료 임계값 이전에 자동 갱신을 시도해야 한다.
...
시스템은 토큰 갱신 실패 시 재시도 가능 상태를 기록하고 사용자에게 재연결 안내를 제공해야 한다.
