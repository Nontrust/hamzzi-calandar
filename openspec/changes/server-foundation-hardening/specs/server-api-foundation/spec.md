...
시스템은 모든 서버 API 응답에서 성공/실패를 동일 구조(`success`, `data`, `errorCode`, `errorMessage`, `requestId`)로 반환해야 한다.
...
시스템은 비즈니스 로직 실행 전에 사용자 인증과 역할 권한 검증을 수행해야 한다.
...
시스템은 각 요청에 추적 ID를 부여하고 주요 보안/권한 이벤트를 감사 로그로 기록해야 한다.
