...
시스템은 최소 1개 이상의 설정된 Open API 소스에서 일정 데이터를 정기적으로 수집해야 하며, 수집 실패 시 재시도 가능한 상태를 기록해야 한다.
...
시스템은 연결된 사용자의 캘린더 이벤트를 업서트하고, 실패 시 서버단 재시도/백오프/최대 횟수 정책을 적용한 상태 전이를 추적해야 한다.
...
시스템은 클라이언트에 실패 원인과 복구 행동을 해석 가능한 표준 실패 코드로 반환해야 한다.
