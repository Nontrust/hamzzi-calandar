...
시스템은 외부 연동 실패 작업에 대해 지수 백오프 기반 재시도 정책을 적용해야 한다.
...
시스템은 작업 상태를 `pending`, `failed`, `retrying`, `synced`로 일관되게 기록해야 한다.
...
시스템은 최대 재시도 횟수 초과 시 무한 반복을 중단하고 운영자 확인 대상 상태로 전환해야 한다.
